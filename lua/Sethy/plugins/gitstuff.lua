return {
        ...
        config = function()
            ...
            autocmd("BufWinEnter", {
                ...
                callback = function()
                    ...
                    local bufnr = vim.api.nvim_get_current_buf() -- buffer number
    ...
    },
	{
		...
		event = { "BufReadPre", "BufNewFile" }, -- Lazy loaded = BufReadPre -> tesne, pred otevrenim existujiciho souboru, BufNewFile - pri otevreni noveho souboru.
		opts = {
			on_attach = function(bufnr)
				...
				-- Navigation
				map("n", "]h", gs.next_hunk, "Next Hunk") -- map("n") -> normal mode map("v") -> visual mode
